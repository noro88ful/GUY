...
<div class="main-content-inner">
	<table id="Info" tblName="Info" class="table table-striped table-dark table-bordered" style="width: 100%">
		<thead>
				<tr>
					<th scope="col" fname="copy">Copyright</th>
					<th scope="col" fname="image">Լոգո</th>
		...
		<tbody>
				...
				<tr class='request_tr' trID="{{$index->id}}">
					<td fvalue="{{$index->copy}}">{{$index->copy}}</td>
					<td fvalue="{{$index->image}}">
						<img src="/files/Info/{{$index->image}}" width="200" alt="">
					</td>
    ...
    <form method="post" action="/add_table_field" class="modal-content" enctype="multipart/form-data">
	 {{csrf_field()}}	
      ...
      <div class="modal-body forum_body">
			<input type="hidden" name="tblname" value="Info">
			<div class="form-group"><input type="text" class="form-control" placeholder="Copyright" name="copy"></div>
...
<script>
	$(window).on('load',function(){
		$('.breadcrumb_span').html('Այլ Տվյալներ')
	})
</script>
@endsection('main')
