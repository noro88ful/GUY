...
<div class="main-content-inner">
	<table id="Contact" tblName="Contact" class="table table-striped table-dark table-bordered" style="width: 100%">
		<thead>
				<tr>
		...
		<tbody>
				...
				<tr class='request_tr' trID="{{$contact->id}}">
					<td fvalue="{{$contact->title}}">{{$contact->title}}</td>
					<td fvalue="{{$contact->text}}">{{$contact->text}}</td>
					<!-- <td fvalue="{{$contact->image}}"><img  width="200" src="/files/Contact/{{$contact->image}}" alt=""></td> -->
    ...
    <form method="post" action="/add_table_field" class="modal-content" enctype="multipart/form-data">
	 {{csrf_field()}}	
      ...
      <div class="modal-body forum_body">
			<input type="hidden" name="tblname" value="Contact">
			<div class="form-group"><input type="title" class="form-control" placeholder="Անուն" name="title"></div>
			<div class="form-group"><input type="text" class="form-control" placeholder="Տեքստ" name="text"></div>
...
<script>
	$(window).on('load',function(){
		$('.breadcrumb_span').html('Կապի տվալներ')
	})
</script>
@endsection('main')
