...
<div class="main-content-inner">
	<table id="Audio" tblName="Audio" class="table table-striped table-dark table-bordered" style="width: 100%">
		<thead>
				<tr>
					<th scope="col" fname="audio">Երաժշտություն</th>
					<th scope="col">Խմբագրել</th>
				</tr>
		</thead>
		<tbody>
				@foreach($audios as $index)
				<tr class='request_tr' trID="{{$index->id}}">
					<td fvalue="{{$index->audio}}">{{$index->audio}}</td>
    ...
    <form method="post" action="/add_table_field" class="modal-content" enctype="multipart/form-data">
	 {{csrf_field()}}	
      ...
      <div class="modal-body forum_body">
			<input type="hidden" name="tblname" value="Audio">
			<div class="form-group">
				<label>Երաժշտություն</label>
...
<script>
	$(window).on('load',function(){
		$('.breadcrumb_span').html('Կայքի երաժշտություն')
	})
</script>
@endsection('main')
