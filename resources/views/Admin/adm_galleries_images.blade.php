...
<div class="main-content-inner">
	<table id="Galleryimage" tblName="Galleryimage" class="table table-striped table-dark table-bordered" style="width: 100%">
		<thead>
				<tr>
					<th scope="col" fname="gallery_id">Աշխատանք</th>
		...
		<tbody>
			@foreach($galleryimages as $tour)
			<tr class='request_tr' trID="{{$tour->id}}">
				<td fvalue="{{$tour->gallery_id}}">{{$tour->gallery->topic}}</td>
				<td fvalue="{{$tour->image}}">
				<a href="tour-details/{{$tour->tour_id}}" target="_blank"><img src="/files/Gallery/{{$tour->image}}" style="width: 100px; height: 100px; object-fit:cover" alt=""></a>
				</td>
				<td fvalue="{{$tour->created_at}}">{{$tour->created_at}}</td>
	...
	<div class="col-lg-12">
		<div class="pagination__number blog__pagination">
			{{$galleryimages->links('pagination.default')}}
		</div>
	</div>
</div>
    ...
    <form method="post" action="/add_table_field" class="modal-content" enctype="multipart/form-data">
	 {{csrf_field()}}	
      ...
      <div class="modal-body forum_body">
			<input type="hidden" name="tblname" value="Galleryimage">
			<p>Նկար</p>
			<div class="form-group"><input type="file" class="form-control" placeholder="Տվյալի անուն" name="imagetour"></div>
			<div class="form-group"><input type="hidden" class="form-control" name="gallery_id" value="{{$galleryimages[0]->gallery_id}}"></div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Չեղարկել</button>
...
<script>
	$(window).on('load',function(){
		$('.breadcrumb_span').html('Աշխատանքի նկարներ')
	})
</script>
@endsection('main')
